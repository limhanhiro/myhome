...
import com.example.thymeleafex.service.Crawling2Service;
...
public class Crawling2Controller {
    private final Crawling2Service service;

    @Autowired
    public Crawling2Controller(Crawling2Service service){
    ...
    public ModelAndView search(SearchRequestDTO dto, int page, String field,@RequestParam(required = false) String sort
        ...
        for(int c=0; c < temp1.length;  c++){
            ...
            if(c+1 == temp1.length){
                if(check.equals(temp1[c])){
                    ...
                    int vali = service.crawling2chkEng(check,dto);//변환 한 값이 의미 있는 검생어 인진 check
                    if(vali >0){
                ...
                if(check.equals(temp1[c])){
                    ...
                    int vali = service.crawling2chkEng(check,dto);
                    if(vali >0){
                        eng.append(check+ " ");
                    }else {
                        eng.append(temp1[c]+ " ");
        ...
        if (searchTerm.contains("-")) {
            ...
            for(int i=0; i<test.length;i++){
                ...
                String result = service.crawling2correctTrans(dto);
            ...
            for(int j=0; j < test1.length; j++){
                ...
                String result = service.crawling2stopTrans(dto);
                if(j+1 == test1.length){
                    if(result == null){
            ...
            if (field.equals("t")) {
                ...
                fields.add(field1);
                dto.setFields(fields);
                ModelAndView mvAll = new ModelAndView("/elastic/crawling2List");
                final int size = 10; // 한페이지에 몇개 보일지 결정
                dto.setSize(size);
                dto.setPage(page);

                List<Crawling2> crawling2 = service.crawling2search(dto);
                ...
                List<Popular> populars = service.crawling2popular(pdto);
                System.out.println(populars.size());
                mvAll.addObject("populars", populars);
                mvAll.addObject("crawling2", crawling2);
            ...
            ModelAndView mvSearch = new ModelAndView("/elastic/crawling2List");
            //정렬 조건 나누기
            if (sort.equals("rec")) {
            ...
            int total = service.crawling2totalCount(dto).size();
            final int size = 10;
            dto.setSize(size);
            dto.setPage(page);
            List<Crawling2> crawling2 = service.crawling2search(dto);
            //검색 로그 쌓기
            if (crawling2.size() > 0 && log.equals("^^")) {
                ...
                searchLog.setDomain("crawling2");
                Boolean result = service.crawling2putSearchLog(searchLog);
                if (result) {
...
//        인기 검색어 가져오기
            ...
            mvSearch.addObject("crawling2", crawling2);
    ...
    public ModelAndView getById(String ID){
        ModelAndView mv = new ModelAndView("/elastic/crawling2Form");
        Crawling2 crawling2 = service.crawling2getById(ID);
        mv.addObject("crawling2",crawling2);
        return mv;
    ...
    public List<AutoComplete> autoComplete(@ModelAttribute SearchRequestDTO dto){
        ...
        return service.crawling2autoComplete(dto);
    }
    ...
    public ModelAndView All(SearchRequestDTO dto, int page){
        ...
        List<Crawling2> crawling2 = service.crawling2All(dto);
        int startPage = Math.max(1,page-4);
        int endPage = page+4;
        String total = "all";
        mv.addObject("crawling2",crawling2);
